...
var pizzaElementGenerator = function(i) {
  ...
  pizzaImage.src = "images/pizza_1.png";
...
function updatePositions() {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    //items[i].style.transform = 'translateX(' + (items[i].basicLeft + 100 * phase) + 'px)'; // 尝试用transform来改变动画实现形式，发现对速度改进没什么卵用
  }
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  // 减少生成披萨滑窗数，200个怎么也太多了吧！
  for (var i = 0; i < 32; i++) {
    var elem = document.createElement('img');
    elem.className = 'mover';
    elem.src = "images/pizza_1.png";
